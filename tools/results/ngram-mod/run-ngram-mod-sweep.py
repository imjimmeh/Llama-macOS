...
def simple_command(args: argparse.Namespace, size: str, cache: Path) -> list[str]:
    command = [
        str(args.simple),
        "-m", str(args.model),
...
def main() -> int:
    ...
    rows = []
    for size in sizes:
        cache = args.output_dir / f"cache-{prefix}-{size}.ngrammod"
        if cache.exists():
            cache.unlink()

        for mode in ("cold", "warm"):
            log_path = args.output_dir / f"{prefix}-{size}-{mode}.log"
            command = simple_command(args, size, cache)
            print(f"running {mode} {size} ...")
    ...
    print()
    print("summary:")
    print(f"  {'size':>8} {'mode':>5} {'lookups':>9} {'hits':>8} {'hit_rate':>9} "
          f"{'overwrite':>10} {'tps':>7}")
    for row in rows:
        if "error" in row:
            continue
        print(f"  {row['size']:>8} {row['mode']:>5} {row['lookups']:>9} {row['hits']:>8} "
              f"{row['hit_rate']:>9.4f} {row['overwrite_rate']:>10.4f} {row['tps']:>7.2f}")
