...
// Warp-level scatter kernel merging GPU hits and uploaded CPU misses into canonical route layout
__global__ void moe_hetero_scatter_kernel(
        ...
        const int n_misses,
        const int d_model,
        const int64_t stride_route_floats) {
    ...
    float * dst_route = dst + (size_t)route_idx * stride_route_floats;
...
void ggml_cuda_moe_scatter_routes(
    ...
    moe_hetero_scatter_kernel<<<grid_size, block_size, 0, stream>>>(
        dst,
        gpu_hits,
        cpu_misses_uploaded,
        hit_route_indices,
