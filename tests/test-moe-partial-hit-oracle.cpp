...
static error_stats compute_error(const float * actual, const float * expected, size_t n, double tol_nmse = 0.01, double tol_mean_rel = 0.05) {
    double max_abs = 0.0;
    double max_rel = 0.0;
    double sum_abs = 0.0;
    double sum_rel = 0.0;
    ...
    // Passed if within NMSE tolerance, mean relative error tolerance, or exact small abs error
    const bool passed = (nmse <= tol_nmse) || (mean_rel <= tol_mean_rel) || (max_abs <= 1e-3);
...
// Story 1.1: Compute CPU reference for all 8 routes
static void compute_cpu_reference(
    ...
    printf("  [DEBUG] CPU ref down[0][0..4]: ");
    for (int i = 0; i < 5; i++) printf("%.4f ", out_unweighted_down[i]);
    printf("\n");
...
// Story 1.2: Compute GPU reference using slot pools for all 8 routes
static void compute_gpu_reference(
    ...
    // Extract unweighted down results
    ggml_backend_tensor_get(down, out_unweighted_down.data(), 0, spec.top_k * spec.d_model * sizeof(float));

    // Debug: print first 5 elements of down for route 0 on CPU vs GPU
    printf("  [DEBUG] CPU ref down[0][0..4]: ");
    for (int i = 0; i < 5; i++) printf("%.4f ", out_unweighted_down[i]);
    printf("\n");
...
// Helper: execute K hit routes on GPU using slot pools
...
// Helper: execute M miss routes on CPU using host weights
...
// Story 1.3: Mixed Partial-Hit Execution for arbitrary N in [0, 8]
static void compute_mixed_partial_hit(
    moe_oracle_context & moec,
    int32_t n_gpu_hits,
    ...
    const auto & spec = moec.spec;
    const int32_t n_misses = (int32_t)spec.top_k - n_gpu_hits;

    out_scattered_down.resize(spec.top_k * spec.d_model);
    out_weighted_down.resize(spec.top_k * spec.d_model);
    out_moe_reduced.assign(spec.d_model, 0.0f);
    ...
    for (int32_t r = 0; r < spec.top_k; r++) {
        moe_route_desc desc;
        desc.token = 0;
        desc.route = r;
    ...
    // 4. Downstream weighting and sum reduction
    for (int64_t r = 0; r < spec.top_k; r++) {
        const float w = moec.routing_weights[r];
        for (int64_t d = 0; d < spec.d_model; d++) {
...
// Story 13: Latency benchmark for given N GPU routes
static bench_stats benchmark_mixed_latency(
    moe_oracle_context & moec,
    int32_t n_gpu_hits,
    int warmup_reps,
    int bench_reps) {
    ...
    for (int i = 0; i < warmup_reps; i++) {
        compute_mixed_partial_hit(moec, n_gpu_hits, down_out, weighted_out, moe_out);
    }
...
int main(int argc, char ** argv) {
    ...
    printf("================================================================================\n");
    printf("MoE Partial-Hit Heterogeneous Execution Oracle Test (Epic 1 / Stories 1.1-1.3)\n");
    printf("Model: Gate A Spec (d_model=2048, d_ff=512, n_expert=256, top_k=8, TG1)\n");
    printf("CPU Threads: %d | Warmup: %d | Reps: %d\n", n_threads, warmup_reps, bench_reps);
    printf("================================================================================\n\n");
    ...
    // -------------------------------------------------------------------------
    // Story 1.1: CPU Reference
    // -------------------------------------------------------------------------
    printf("--------------------------------------------------------------------------------\n");
    printf("Story 1.1: Compute Full CPU Reference (8/8 CPU Routes)\n");
    printf("--------------------------------------------------------------------------------\n");

    std::vector<float> cpu_ref_unweighted_down;
    std::vector<float> cpu_ref_weighted_down;
    std::vector<float> cpu_ref_moe_reduced;
    compute_cpu_reference(moec, cpu_ref_unweighted_down, cpu_ref_weighted_down, cpu_ref_moe_reduced);
    printf("  CPU Reference computed successfully. (Unweighted down size: %zu elements)\n\n",
        cpu_ref_unweighted_down.size());

    // -------------------------------------------------------------------------
    // Story 1.2: GPU Reference
    // -------------------------------------------------------------------------
    printf("--------------------------------------------------------------------------------\n");
    printf("Story 1.2: Compute Full GPU Reference (8/8 GPU Slot Pool Routes)\n");
    printf("--------------------------------------------------------------------------------\n");

    std::vector<float> gpu_ref_unweighted_down;
    std::vector<float> gpu_ref_weighted_down;
    std::vector<float> gpu_ref_moe_reduced;
    compute_gpu_reference(moec, gpu_ref_unweighted_down, gpu_ref_weighted_down, gpu_ref_moe_reduced);

    error_stats gpu_err_down = compute_error(
        gpu_ref_unweighted_down.data(), cpu_ref_unweighted_down.data(), cpu_ref_unweighted_down.size());
    error_stats gpu_err_moe = compute_error(
        gpu_ref_moe_reduced.data(), cpu_ref_moe_reduced.data(), cpu_ref_moe_reduced.size());

    printf("  GPU vs CPU Unweighted Down Error : NMSE=%.6f, MeanRel=%.4f, MaxAbs=%.4f [%s]\n",
        gpu_err_down.nmse, gpu_err_down.mean_rel_error, gpu_err_down.max_abs_error,
        gpu_err_down.passed ? "PASS" : "FAIL");
    printf("  GPU vs CPU MoE Reduced Output Error: NMSE=%.6f, MeanRel=%.4f, MaxAbs=%.4f [%s]\n\n",
        gpu_err_moe.nmse, gpu_err_moe.mean_rel_error, gpu_err_moe.max_abs_error,
        gpu_err_moe.passed ? "PASS" : "FAIL");

    if (!gpu_err_down.passed || !gpu_err_moe.passed) {
        fprintf(stderr, "FAIL: GPU vs CPU baseline tolerance check failed!\n");
        return 1;
    }

    // -------------------------------------------------------------------------
    // Story 1.3 & Epic 12: Mixed Partial-Hit Execution Matrix for N = 0..8
    // -------------------------------------------------------------------------
    printf("--------------------------------------------------------------------------------\n");
    printf("Story 1.3 & Epic 12: Partial-Hit Mixed Execution Correctness Matrix (N=0..8)\n");
    printf("--------------------------------------------------------------------------------\n");
    printf("  %-12s | %-10s | %-10s | %-12s | %-12s | %-8s\n",
        "Residency", "Down NMSE", "MeanRel", "Down MaxAbs", "MoE MaxAbs", "Status");
    printf("  ------------------------------------------------------------------------------\n");

    bool all_passed = true;

    for (int32_t N = 0; N <= spec.top_k; N++) {
        std::vector<float> mixed_down;
        std::vector<float> mixed_weighted;
        std::vector<float> mixed_moe;

        compute_mixed_partial_hit(moec, N, mixed_down, mixed_weighted, mixed_moe);

        error_stats err_down = compute_error(
            mixed_down.data(), cpu_ref_unweighted_down.data(), cpu_ref_unweighted_down.size());
        error_stats err_moe = compute_error(
            mixed_moe.data(), cpu_ref_moe_reduced.data(), cpu_ref_moe_reduced.size());

        // Exact slice verification: check that GPU slices match GPU ref and CPU slices match CPU ref
        for (int32_t r = 0; r < N; r++) {
            const float * mixed_r = mixed_down.data() + (size_t)r * spec.d_model;
            const float * gpu_r   = gpu_ref_unweighted_down.data() + (size_t)r * spec.d_model;
            for (int64_t d = 0; d < spec.d_model; d++) {
                if (std::abs(mixed_r[d] - gpu_r[d]) > 1e-4f) {
                    err_down.passed = false;
                }
            }
        }
        for (int32_t r = N; r < spec.top_k; r++) {
            const float * mixed_r = mixed_down.data() + (size_t)r * spec.d_model;
            const float * cpu_r   = cpu_ref_unweighted_down.data() + (size_t)r * spec.d_model;
            for (int64_t d = 0; d < spec.d_model; d++) {
                if (std::abs(mixed_r[d] - cpu_r[d]) > 1e-4f) {
                    err_down.passed = false;
                }
            }
        }
        ...
        const bool pass = err_down.passed && err_moe.passed;
        if (!pass) all_passed = false;

        printf("  %-12s | %10.6f | %10.4f | %12.4f | %12.4f | %-8s\n",
            label, err_down.nmse, err_down.mean_rel_error, err_down.max_abs_error,
            err_moe.max_abs_error, pass ? "PASS" : "FAIL");
    }

    printf("\n");
    if (!all_passed) {
        fprintf(stderr, "FAIL: One or more partial-hit configurations failed correctness check!\n");
        return 1;
    }
    printf(">>> All 9 Partial-Hit Correctness Configurations (0/8 through 8/8) PASSED! <<<\n\n");

    // -------------------------------------------------------------------------
    // Epic 13: Partial-Hit Latency Curve Benchmark
    // -------------------------------------------------------------------------
    if (run_bench) {
        printf("--------------------------------------------------------------------------------\n");
        printf("Epic 13: Partial-Hit Execution Latency Curve (N=0..8)\n");
        printf("--------------------------------------------------------------------------------\n");
        ...
        for (int32_t N = 0; N <= spec.top_k; N++) {
            bench_stats s = benchmark_mixed_latency(moec, N, warmup_reps, bench_reps);
            char label[32];
            snprintf(label, sizeof(label), "%d GPU / %d CPU", N, (int)(spec.top_k - N));
    ...
    printf("Oracle Verification Completed Successfully.\n");
    return 0;
}
